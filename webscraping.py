...
def parse_movie_details(html, movie_url):
    ...
    streaming_embed = extract_streamtape_embed(download_links)

    return {
        'title': title.text.strip() if title else 'No Title',
        'poster_url': poster['src'] if poster else None,
        'download_links': download_links,
        'streaming_embed': streaming_embed
    }
...
def scrape_first_page():
    """Scrape only page 1, check for new movies, and update `movies.json`."""
    existing_movies = load_existing_movies()
    existing_titles = {movie['title'] for movie in existing_movies}

    print("🔍 Scraping first page for new movies...")
    page_html = fetch_page(f"{BASE_URL}/page/1/")
    if not page_html:
        print("⚠️ Skipping page 1 due to fetch failure.")
        return
    ...
    # If new movies are found, save them
    if new_movies:
        existing_movies.extend(new_movies)
        save_movies_to_json(existing_movies)
        print(f"✅ Added {len(new_movies)} new movies to the database!")
    else:
